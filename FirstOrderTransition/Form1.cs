            ...
            for (H = -5; H < 5; H = H + 0.1)
            {
                ...
                for(int sw=0;sw<sweeps;sw++)
                {
                    ...
                    for (int i = 1; i < size - 1; i++)
                    {
                        ...
                        {
                            double E1 = -spinsystem[i, j] * (spinsystem[i - 1, j] + spinsystem[i + 1, j]
                                + spinsystem[i, j - 1] + spinsystem[i, j + 1] + mu * H);
                            //E1 before flipping
                            spinsystem[i, j] = spinsystem[i, j] * -1;//flip the spin
                            double E2 = -spinsystem[i, j] * (spinsystem[i - 1, j] + spinsystem[i + 1, j]
                                + spinsystem[i, j - 1] + spinsystem[i, j + 1] + mu * H);
                            ...
                            else
                            {
                                ...
                                {
                                    spinsystem[i, j] = spinsystem[i, j] * -1;//reject the flipping
                                }
                            }
                        }
                    }
            ...
            for (H = -3; H < 3; H = H + 0.1)
            {
                ...
                for (int sw = 0; sw < sweeps; sw++)
                {
                    ...
                    for (int i = 1; i < size - 1; i++)
                    {
                        for (int j = 1; j < size - 1; j++)
                        {
                            Flip1(i, j, spinsystem, H, mu, T, bb, br, gg);
                        }
                    }
                }
                Mave = 0;
                ...
                M = Mave / (size * size - 4 * size + 4);
                gg.FillEllipse(br, 600 + (float)H * 90,
                    600 - (float)M * 40, 5, 5);
            }
            //Start Decreasing Temp
            for (H = 3; H > -3; H = H - 0.1)
            {
                ...
                M = Mave / (size * size - 4 * size + 4);
                gg.FillEllipse(bb, 600 + (float)H * 90,
                    600 - (float)M * 40, 5, 5);
            }


        }
        void Flip1(int i, int j,
            int[,] arr, double H, double mu, double T,
            SolidBrush bb, SolidBrush br, Graphics gg)
        {
            Random obj = new Random();
